...
export const Navbar = ({data}:Navbarprops) => {
  ...
  const handleShare = () => {
    ...
    if (navigator.share) {
      navigator.share({ title: "Document", url });
    } else {
  ...
  const handleCut = () => isMounted && document.execCommand("cut");
  const handleCopy = () => isMounted && document.execCommand("copy");
  const handlePaste = () => isMounted && document.execCommand("paste");
  ...
  const handleToggleSpellCheck = () => {
    ...
    if (editorEl) {
      ...
      editorEl.spellcheck = next;
      editorEl.setAttribute("spellcheck", String(next));
      setIsSpellCheckEnabled(next);
      editor?.chain().focus().run();
      alert(next ? "✓ Spell check enabled" : "✗ Spell check disabled");
    }
  };
          ...
          <Menubar className="border-none bg-transparent shadow-none h-auto p-0">
            ...
            <MenubarMenu>
              ...
              <MenubarContent>
                ...
                <MenubarSeparator />
                <MenubarItem className="gap-2">About G-Docs</MenubarItem>
              </MenubarContent>
            </MenubarMenu>
          </Menubar>
        </div>
